 ...
 */
function buildSessionKey(channelId: string, userId: string, isDM: boolean): string {
  if (isDM) {
 ...
 */
async function shouldRespond(message: any, context: any, config: SlackConfig): Promise<boolean> {
  // Ignore bot messages
  ...
  if (isDM) {
    ...
    if (allowFrom.includes(message.user)) return true;
    
    // TODO: Implement pairing code flow
    // For now, log the unapproved DM
    logger.info({ 
      msg: "Unapproved DM received", 
      user: message.user,
 ...
 */
async function handleMessage(message: any, context: any, say: any, config: SlackConfig) {
  logger.debug({ 
  ...
  const shouldThread = replyToMode === "all" || 
 ...
 */
async function updateMessage(state: StreamState, content: string): Promise<void> {
 ...
 */
async function finalizeMessage(state: StreamState, content: string): Promise<void> {
  if (!app || state.isFinalized) return;
  ...
  async init(context: WOPRPluginContext) {
    ...
    try {
      ...
      app.event("app_mention", async ({ event, context, say }) => {
        await handleMessage(event, { ...context, channel: event.channel }, say, config);
      });
      ...
      logger.error({ msg: "Failed to initialize Slack app", error: error.message });
      throw error;
    }
  },
