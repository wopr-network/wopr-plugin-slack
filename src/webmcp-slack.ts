...
export function registerSlackTools(
	...
	registry.register({
		name: "getSlackStatus",
		description:
			"Get Slack bot status: online/offline, connected workspaces, and latency.",
		parameters: {},
	...
	registry.register({
		...
		parameters: {
			workspaceId: {
				type: "string",
				description:
					"Optional workspace identifier to filter channels. Omit for all workspaces.",
				required: false,
			},
		},
	...
	registry.register({
		...
		description:
			"Get Slack message statistics: messages processed and active conversations.",
		parameters: {},
		handler: async (_params: Record<string, unknown>, auth: AuthContext) => {
			return daemonRequest(
				apiBase,
				`/plugins/${encodeURIComponent("wopr-plugin-slack")}/stats`,
				auth,
			);
		},
	});
